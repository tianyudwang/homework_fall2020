...
class RL_Trainer(object):
    ...
    def collect_training_trajectories(self, itr, load_initial_expertdata, collect_policy, batch_size):
        ...
        if itr == 0 and load_initial_expertdata is not None:
            with open(load_initial_expertdata, 'rb') as path_file:
                loaded_paths = pickle.load(path_file)
            ...
            while envsteps_this_batch <= batch_size:
                path, timesteps = utils.sample_trajectories(
                    self.env, 
                    collect_policy, 
                    max((batch_size - envsteps_this_batch) // self.params['ep_len'], 1), 
                    max_path_length=self.params['ep_len'])
                paths.extend(path)
                envsteps_this_batch += timesteps
